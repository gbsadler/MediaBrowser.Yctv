var Utility = {
        ...
        buttons = [
            {
                ...
                callback: function() {
                    ...
                    MediaBrowserService._serverPingCheck.ticking = true;
                }
            }
        ];
    ...
    mediabrowserServiceDownDialog: new KONtx.dialogs.Alert({
        title: $_('mediabrowserservice_unavail'),
        message: $_('mediabrowserservice_unavail_message'),
        buttons: [
            {
                label: $_('dialog_ok_button'),
                callback: function() {
    ...
    buildMediaArray: function(mediaResp) {
        ...
        if (mediaItems) {
            for ( i = 0; i < mediaItems.length; i++ ) {
                ...
                imageURL = MediaBrowserService.generateImageURL(image, Config.THUMBWIDTH, Config.THUMBHEIGHT, imageFormat);
    ...
    updateMediaMetadata: function (gridData) {
        ...
        if (mediaInfo.contentRating) {
            contentRating = MediaBrowserService.generateImageURL(mediaInfo.mediaTagPrefix + 'contentRating/' + mediaInfo.contentRating + '?t=' + mediaInfo.mediaTagVersion, 40,  40, 'png' );
        }
        ...
        if ($contains( mediaInfo.type, Config.VIDEOMEDIATYPES)) {
            if (mediaInfo.videoResolution) {
                videoResolution = MediaBrowserService.generateImageURL(mediaInfo.mediaTagPrefix + 'videoResolution/' + mediaInfo.videoResolution + '?t=' + mediaInfo.mediaTagVersion, 40,  40, 'png' );
            }
    
            if (mediaInfo.videoCodec) {
                videoCodec = MediaBrowserService.generateImageURL(mediaInfo.mediaTagPrefix + 'videoCodec/' + mediaInfo.videoCodec + '?t=' + mediaInfo.mediaTagVersion, 40,  40, 'png' );
            }
    
            if (mediaInfo.videoFrameRate) {
                videoFramerate = MediaBrowserService.generateImageURL(mediaInfo.mediaTagPrefix + 'videoFrameRate/' + mediaInfo.videoFrameRate + '?t=' + mediaInfo.mediaTagVersion, 40,  40, 'png' );
            }
    
            if (mediaInfo.audioCodec) {
                audioCodec = MediaBrowserService.generateImageURL(mediaInfo.mediaTagPrefix + 'audioCodec/' + mediaInfo.audioCodec + '?t=' + mediaInfo.mediaTagVersion, 40,  40, 'png' );
            }
    
            if (mediaInfo.audioChannels) {
                audioChannels = MediaBrowserService.generateImageURL(mediaInfo.mediaTagPrefix + 'audioChannels/' + mediaInfo.audioChannels + '?t=' + mediaInfo.mediaTagVersion, 40,  40, 'png' );
            }
        }

        //TODO: Need to calculate height better. Can we find it from the PMS? At LEAST, put it in config.
        if (mediaInfo.art) {
            artImage = MediaBrowserService.generateImageURL(mediaInfo.art, this.width - 10,  parseInt(((this.width/3)*2) - 10, 10), 'jpg' );
        }
        else {
            artImage = null;
        }
    ...
    mediaTypeDispatcher: function(mediaInfo) {
        ...
        KONtx.utility.timer.setTimeout( (function(){
            ...
            if(mediaInfo) {
                ...
                else if ( (!mediaInfo.type) && ( $contains(mediaInfo.viewGroup, Config.SEASONMEDIATYPES) || MediaBrowserService.useChannels ) ) {
...
//            focusOnCompletion: this.controls.pageIndicator,
            buttons: [
                {
                    label: $_('resume_time_label', Utility.millisecondsToTime( MediaBrowserService.getCurrentMediaOffset() )),
                    callback: (function() {
    ...
    prePlayMovie: function() {
        ...
        if ( ! MediaBrowserService.getCurrentMediaAvailable() ) {
