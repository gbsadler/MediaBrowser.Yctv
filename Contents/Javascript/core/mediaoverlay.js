    ...
    ***/
    _onSourceUpdated: function (event) {
            switch (event.type) {
                ...
                if ( event.payload.newState == KONtx.mediaplayer.constants.states.PLAY ) {
                    ...
                    if ( this._rwffJustFinished && ((!Config.get( 'activerwff' )) || MediaBrowserService.useChannels) ) {
                        this._rwffJustFinished = false;
    ...
    _goForward: function() {
        ...
        if ( timeIndex  <= KONtx.mediaplayer.tvapi.currentMediaDuration ) {
            ...
            if (  Config.get( 'activerwff' ) && (!MediaBrowserService.useChannels) ) {
                if ( this._rwffLoop === 4 ) {
            ...
            //MediaBrowserService.playState(KONtx.mediaplayer.tvapi.currentMediaDuration, KONtx.mediaplayer.tvapi.currentMediaDuration, KONtx.mediaplayer.tvapi.currentPlayerState );
            this._rwff = false;
            this._rwffMultiplier = 1;
            this._rwffOffset = 0;
            this._registerPlayerHandlers();
